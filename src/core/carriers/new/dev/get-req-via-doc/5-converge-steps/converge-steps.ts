...
export const convergeSteps = async (serviceName: string): Promise<void> => {
  ...
  const serviceDir = path.join(
    `./src/core/carriers/new/dev/get-req-via-doc/4b-process-links/2-process-content/extracted-steps/${formattedServiceName}`,
  );
  ...
  const outputDir = `./src/storage/carriers/new/dev/data-on-steps`;
