...
const saveJSON = async (filePath: string, data: any): Promise<void> => {
  try {
    ...
    const logDirectory =
      './src/core/carriers/new/dev/get-req-via-doc/logs/errors/3-scrape-links';
...
export const scrapeLinks = async (
  ...
  const serviceDir = path.join(
    './src/core/carriers/new/dev/get-req-via-doc/3-scrape-links/scraped-links/',
  ...
  try {
    const inputLinksFilePath = `./src/core/carriers/new/dev/get-req-via-doc/2-filter-scraped-site/filtered-links/filtered_links_${serviceName
      .toLowerCase()
      .replace(/\s+/g, '_')}.json`;
