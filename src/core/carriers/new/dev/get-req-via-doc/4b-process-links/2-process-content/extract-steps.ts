...
async function _extractSteps(
  ...
  try {
    ...
    const logDirectory =
      './src/core/carriers/new/dev/get-req-via-doc/logs/errors/4b-process-links';
...
export async function extractSteps(
  ...
  try {
    ...
    const directoryPath = `./src/core/carriers/new/dev/get-req-via-doc/4b-process-links/1-extract-content-from-scraped-links/extracted-content/${formattedServiceName}`;

    const jsonFiles = fs
      .readdirSync(directoryPath)
      .filter((file) => file.endsWith('.json'));

    const outputDirectory = `./src/core/carriers/new/dev/get-req-via-doc/4b-process-links/2-process-content/extracted-steps/${formattedServiceName}`;
