...
export const extractContent = async (serviceName: string): Promise<void> => {
  ...
  const serviceDir = path.join(
    `./src/core/carriers/new/dev/get-req-via-doc/3-scrape-links/scraped-links/${formattedServiceName}`,
  );
  if (!fs.existsSync(serviceDir)) {
    fs.mkdirSync(serviceDir, { recursive: true });
  }

  const outputDir = `./src/core/carriers/new/dev/get-req-via-doc/4b-process-links/1-extract-content-from-scraped-links/extracted-content/${formattedServiceName}`;
