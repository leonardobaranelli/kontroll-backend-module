...
async function _extractSteps(
  ...
  try {
    ...
    const logDirectory =
      './src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/logs/errors/4b-process-links';
...
export async function extractSteps(
  ...
  try {
    ...
    const directoryPath = `./src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/4b-process-links/1-extract-content-from-scraped-links/extracted-content/${formattedServiceName}`;

    const jsonFiles = fs
      .readdirSync(directoryPath)
      .filter((file) => file.endsWith('.json'));

    const outputDirectory = `./src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/4b-process-links/2-process-content/extracted-steps/${formattedServiceName}`;
