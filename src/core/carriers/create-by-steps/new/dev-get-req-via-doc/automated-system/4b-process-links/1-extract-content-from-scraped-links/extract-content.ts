...
export const extractContent = async (serviceName: string): Promise<void> => {
  ...
  const serviceDir = path.join(
    `./src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/3-scrape-links/scraped-links/${formattedServiceName}`,
  );
  if (!fs.existsSync(serviceDir)) {
    fs.mkdirSync(serviceDir, { recursive: true });
  }

  const outputDir = `./src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/4b-process-links/1-extract-content-from-scraped-links/extracted-content/${formattedServiceName}`;
