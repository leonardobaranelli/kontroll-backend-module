...
export const convergeSteps = async (serviceName: string): Promise<void> => {
  ...
  const serviceDir = path.join(
    `./src/core/carriers/create-by-steps/new/dev-get-req-via-doc/automated-system/4b-process-links/2-process-content/extracted-steps/${formattedServiceName}`,
  );
  ...
  const outputDir = `./src/storage/carriers/new/dev-get-req-via-doc/data-on-steps`;
