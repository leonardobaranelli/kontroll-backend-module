...
import { IConnectorPublic, IError } from '../utils/types/utilities.interface';
...
export default class ConnectorService {
  ...
  public static async getConnectorByType(
    ...
    try {
      const connectorsCollection = getConnectorsCollection();
      const snapshot = await connectorsCollection
        .where('type', '==', type)
        .get();
  ...
  public static async deleteConnectorById(id: string): Promise<void> {
    try {
      ...
      await batch.commit();
      console.log(`Connector with ID ${id} deleted succesfully`);
    } catch (error: unknown) {
      throw error;
    }
  }
